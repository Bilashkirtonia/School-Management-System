...
class studentMonthlyFeeController extends Controller
{
    public function month(){
        ...
        return view('admin.monthlyFee.reg_view',$data);
    }

    public function monthstudent(Request $request){
        ...
        $html['thsource'] .='<th> Month name</th>';
        $html['thsource'] .='<th> Action</th>';
        
        

        foreach($AssingStudent as $key => $value){
            $resgistrationFee = fee_category::where('fee_type_id','1')->where('class_id',$value->class_id)->first();
            $color ='success';
            ...
            $html[$key]['tdsource'] .= '<a href="'.route('setup.student.reg.monthlyFee.payslip').'?class_id='.$value->class_id.'&student_id='.$value->student_id.'&month='.$month.'" class="btn btn-sm btn-'.$color.'" title="payslip" target="_blank">Payslip</a>';
            $html[$key]['tdsource'] .='</td>';
        }
    ...
    public function payslip(Request $request){
        ...
        $data['fee_category'] = fee_category::where('fee_type_id','1')->where('class_id',$class_id)->first(); 
        $pdf = Pdf::loadView('admin.monthlyFee.pdf',$data);
        return $pdf->stream();
    }
}
